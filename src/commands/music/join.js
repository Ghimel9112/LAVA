...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (!permissions.has('Connect') || !permissions.has('Speak')) {
            return interaction.reply({ content: '❌ I need **Connect** and **Speak** permissions in that channel!', ephemeral: true });
        }

        // Check if already in a channel
        const existing = interaction.client.shoukaku.players.get(interaction.guild.id);
        if (existing) {
            return interaction.reply({ content: '❌ I\'m already connected to a voice channel.', ephemeral: true });
        }
